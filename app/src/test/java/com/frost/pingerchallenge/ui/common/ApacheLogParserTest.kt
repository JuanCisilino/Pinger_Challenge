...
class ApacheLogParserTest {
    ...
    fun stringIsParsedCorrectly() {
        ...
        Assert.assertEquals("/team/, /products/desk/", parsedMap["123.4.5.9"]?.joinToString())
        Assert.assertEquals("/products/car/", parsedMap["123.4.5.6"]?.joinToString())
        Assert.assertEquals("/products/desk/", parsedMap["123.4.5.8"]?.joinToString())
        Assert.assertEquals("/team/", parsedMap["123.4.5.4"]?.joinToString())
    }

    @Test
    fun sizeOfListIsCorrect() {
        val parsedMap = parser.parseLogsForEachUser()
        Assert.assertEquals(2, parsedMap["123.4.5.9"]?.size)
        Assert.assertEquals(1, parsedMap["123.4.5.6"]?.size)
        Assert.assertEquals(1, parsedMap["123.4.5.8"]?.size)
        Assert.assertEquals(1, parsedMap["123.4.5.4"]?.size)
    }
}